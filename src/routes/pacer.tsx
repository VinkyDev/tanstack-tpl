    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6 sm:p-10">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground">
          防抖 (Debounce) / 节流 (Throttle) / 速率限制 (Rate Limit) — 控制函数与状态的执行频率
        </p>
      </header>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <div className="space-y-4 p-6 pt-0">
          <div className="space-y-1">
            <label className="text-muted-foreground text-xs">原始输入</label>
            <Input
              value={input}
          ...
          <div className="space-y-1">
            <label className="text-muted-foreground text-xs">防抖后</label>
            <div className="rounded-lg border bg-muted/50 p-3 font-mono text-sm">
              {debouncedInput || <span className="text-muted-foreground/50">等待输入...</span>}
            </div>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-2 p-6 pt-0">
          ...
          <StateRow label="isPending" value={debouncer.state.isPending ? "true" : "false"} />
          <StateRow label="executionCount" value={String(debouncer.state.executionCount ?? 0)} />
        </div>
      </Card>
    </div>
  );
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        ...
        <div className="space-y-2 p-6 pt-0">
          ...
          <StateRow label="executionCount" value={String(throttler.state.executionCount ?? 0)} />
          <StateRow
            label="nextExecutionTime"
            value={
              throttler.state.nextExecutionTime
                ? new Date(throttler.state.nextExecutionTime).toLocaleTimeString()
                : "—"
            }
          />
        </div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        ...
        <div className="space-y-2 p-6 pt-0">
          ...
          <StateRow label="executionCount" value={String(limiter.state.executionCount ?? 0)} />
          <StateRow label="rejectionCount" value={String(limiter.state.rejectionCount ?? 0)} />
        </div>
      </Card>
    </div>
  );
